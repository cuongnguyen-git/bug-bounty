...
All recon commands run directly in Kali WSL. No SSH.
...
```bash
echo '$ARGUMENTS' | ~/go/bin/waybackurls | grep '\.js' | grep -v '\.json' | sort -u
echo '$ARGUMENTS' | ~/go/bin/gau --blacklist png,jpg,gif,svg,css,woff,woff2 | grep '\.js' | sort -u
~/go/bin/katana -u '$ARGUMENTS' -jc -d 3 -silent | grep '\.js' | sort -u
```

If a tool is missing, install it before proceeding — do not stop and ask:
...
Download each first-party JS file to a working directory in Kali WSL. Never write to /mnt/c/.

```bash
mkdir -p ~/bugbounty/js-analysis/[target-slug]
cd ~/bugbounty/js-analysis/[target-slug]
cat urls.txt | xargs -I{} sh -c 'curl -s -o "$(echo {} | md5sum | cut -d" " -f1).js" "{}"'
```
